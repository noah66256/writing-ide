...
当用户说"复盘这个 bug"、"深度排查"、"用 bug 复盘流程"时，按以下阶段执行。**终点是 spec 文档，不写代码。**

详细流程文档：`docs/specs/skill-bug-retrospective-v0.1.md`

### 阶段

1. **文档索引**：扫描近 10 天 `docs/specs/` 和 `docs/research/` 中与 bug 相关的文档，建立上下文，避免重复调查
2. **Commit 回顾**：`git log --oneline -20`，筛选嫌疑 commit（尤其是迁移/重构），检查是否遗漏旧逻辑
3. **症状收集**：错误码原文、复现条件、服务器日志、部署版本确认
4. **代码考古**：从错误码反向追踪调用链，穷举所有代码路径，标注安全/有问题
5. **根因定位**：Claude 写问题描述 → Codex 深度分析 → 多轮讨论确认主因/次因/同类受害者
6. **输出 spec 文档**：写入 `docs/specs/fix-{bug-name}-v1.md`，包含分级根因、每个 Fix 的 diff patch、验证 checklist
...
- 错误码是入口不是答案，预期多层问题叠加
- 同类受害者分析：发现一个就查所有同类
- spec 写完交棒给 Codex 执行，不在分析阶段写代码

---

## 需求分析流程（Feature Spec）

当用户说"分析这个需求"、"帮我出个实施方案"、"写个 feature spec"时，按以下阶段执行。**终点是 spec 文档，不写代码。**

详细流程文档：`.claude/skills/feature-spec/SKILL.md`

### 阶段

1. **诉求澄清**：提取场景/目标/对标/约束，输出需求卡片，经用户确认
2. **项目现状分析**：文档索引 + commit 历史 + 代码探索，输出现状地图（相关文件、已有设施、约束点）
3. **外部调研**：对标产品 + 全网技术方案 + 上游 Issue/RFC，输出调研摘要
4. **方案设计 + Codex 协作**：Claude 出草案 → Codex 审视 + 出 diff → 多轮至共识（可行性/改动范围/边界/连锁反应）
5. **影响评估**：连锁反应排查 + 回归风险评估，输出影响矩阵
6. **输出 spec 文档**：写入 `docs/specs/feat-{name}-v1.md`，包含精确行号、diff patch、验证 checklist

### 原则

- 需求先于方案，现状先于设计，调研先于拍脑袋
- 连锁反应是重点：每个改动必须评估对现有功能的影响
- 最小改动原则：能复用现有设施不造新轮子
- spec 写完交棒给 Codex 执行，不在分析阶段写代码
