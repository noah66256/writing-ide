...
export const TOOL_LIST: ToolMeta[] = [
  ...
  {
    name: "edit",
    description: "对指定文件进行精确的字符串替换编辑（支持 Undo 回滚）。",
    args: [
      { name: "path", required: false, desc: "文件路径（默认 activePath）", type: "string" },
      { name: "old_string", required: false, desc: "要替换的原文本（必须在文件中唯一匹配）", type: "string" },
      { name: "new_string", required: false, desc: "替换后的新文本", type: "string" },
      { name: "replace_all", required: false, desc: "是否替换所有匹配（默认 false，仅替换第一个）", type: "boolean" },
      { name: "edits", required: false, desc: "JSON 数组：TextEdit[]（Monaco range 模式，高级用法）", type: "array" },
    ],
    modes: ["agent"],
    inputSchema: {
      type: "object",
      properties: {
        ...
        edits: { type: "array", items: { type: "object" }, description: "Monaco TextEdit 数组（高级用法）" },
      },
      required: [],
      additionalProperties: true,
    },
