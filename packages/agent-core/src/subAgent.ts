...
export const BUILTIN_SUB_AGENTS: SubAgentDefinition[] = [
  ...
  },
  {
    ...
    systemPrompt: [
      ...
      "",
      "⚠ 重要：你必须在第一轮就调用工具，不要输出任何解释或计划文本。直接调用 kb.import 开始工作。",
      "",
      "编排流程：",
      "1. 直接调用 kb.import(text=用户提供的全部文本) → 拿到 { libraryId, docIds }",
      "2. kb.extract({ docIds, autoPlaybook: true, autoAttach: true }) → 触发后台抽卡",
      "3. kb.jobStatus → 确认入队",
      "4. 一句话汇报结果",
      "",
      "规则：",
