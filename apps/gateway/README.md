...
走 git pull → workspace-scoped `npm install -w @ohmycrab/gateway -w @ohmycrab/admin-web` → `npm -w @ohmycrab/shared run build` → gateway build → pm2 restart → health check。
