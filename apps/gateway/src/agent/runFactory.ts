...
export function buildAgentProtocolPrompt(args: {
  ...
  const modePolicy =
    mode === "chat"
      ...
      : `当前模式：Agent（直接执行）。\n` +
        ...
        `- 任务完成后必须调用 run.done 结束；纯文本说"已完成"不能结束任务，只有 run.done 才能真正结束。不要在 run.done 前反复确认"已完成""无需重跑"。\n` +
  ...
  return (
    ...
    `- 不要在工具调用前加冒号（工具调用可能不会直接显示在输出中）。\n` +
    `- 用户说中文时用中文回复，说英文时用英文回复。不要在对话中无故切换语言。\n\n` +
    // ── 输出效率 ──
    `# 输出效率\n` +
    `直奔重点。先试最简单的方法，不要绕弯子。\n` +
    `保持文字输出简短直接。先给结论或行动，再给推理过程。跳过填充词、铺垫和不必要的过渡。不要复述用户说过的话——直接做。\n` +
    `重点输出：需要用户决策的选项、里程碑式的进度更新、改变计划的错误或阻碍。\n` +
    `一句话能说清的不要用三句。此规则不适用于工具调用。\n\n` +
    // ── 身份与能力 ──
