...
import { BookOpen, ChevronLeft, ChevronRight, FileText, Folder, FolderOpen } from "lucide-react";
...
type ActiveGroup = null | "files" | "kb";

/** 统一 action 模型：键盘导航不区分分组入口 / 目录 / mention 选项 */
type PopoverAction =
  | { key: string; kind: "group"; group: "files" | "kb" }
...
export function MentionPopover({ query, visible, onSelect, onClose }: Props) {
  ...
  const searchKb = useMemo(
    () =>
      searching
        ? libraries
            .filter((lib) => `${lib.id} ${lib.name} ${lib.purpose} ${PURPOSE_LABEL[lib.purpose] ?? lib.purpose}`.toLowerCase().includes(q))
            .map((lib): MentionItem => ({ id: lib.id, type: "kb", label: lib.name, icon: <BookOpen size={14} /> }))
        : [],
    [searching, q, libraries],
  );
  ...
  }, [searching, searchFiles, searchKb, activeGroup, groupEntries, kbItems, curEntries]);
      ...
      <div className="max-h-[240px] overflow-y-auto py-1.5">
        {searching ? (
          <>
            ...
            {searchKb.map((item) => (
              <MentionRow key={item.id} item={item} selected={selectedKey === `m:${item.type}:${item.id}`} onSelect={onSelect} colorClass="text-blue-500" />
            ))}
          </>
        ) : activeGroup === null ? (
