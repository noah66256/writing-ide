...
const MAX_LOGS = 200;
const SUPPORTED_ITEM_TYPES = new Set(["skill", "mcp_server"]);
...
export class MarketplaceManager {
  ...
  async _loadState() {
    try {
      ...
      const rawInstalled = parsed.installed && typeof parsed.installed === "object" ? parsed.installed : {};
      state.installed = Object.fromEntries(
        Object.entries(rawInstalled).filter(([, value]) =>
          SUPPORTED_ITEM_TYPES.has(String(value?.type ?? "").trim()),
        ),
      );
      state.logs = Array.isArray(parsed.logs) ? parsed.logs : [];
      if (Object.keys(state.installed).length !== Object.keys(rawInstalled).length) {
        await this._saveState(state).catch(() => void 0);
      }
      return state;
    } catch {
