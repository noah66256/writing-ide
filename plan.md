...
- [Agent 流程系统性审计与改造路线图（路线2）](docs/specs/agent-flow-audit-roadmap.md)
- [Intent Routing（第一道门禁）](docs/specs/intent-routing-v0.4.md)
- [意图识别/路由模块成熟方案调研 v1（research）](docs/research/intent-routing-modules-survey-v1.md)
- [Vibe Coding Playbook（面向”非专业但有逻辑”的开发者）](docs/vibe-coding-playbook.md)
- [IDE 工具与全项目搜索（研究）](docs/research/ide-tools-search-and-file-ops-v1.md)
- [Web Search 选型与落地路径（研究 v1）](docs/research/web-search-v1.md)
- [Cursor 类 IDE 多模型接口适配范式（research v1）](docs/research/cursor-like-multi-model-adapters-v1.md)
...
- [Context Pack Selector v0.1（spec：1.5B 小模型决定“注入哪些上下文”）](docs/specs/context-pack-selector-v0.1.md)
...
- [lint.style 超时治理：快失败 + 自动切换模型 + prompt 控量（research v1）](docs/research/lint-style-timeout-mitigation-v1.md)
...
- [KB Manager V2（更像原文）方案草案](docs/specs/kb-manager-v2-spec.md)
- [Style Selector v1（自动选簇/选21卡，按话题适配，模型解耦）](docs/specs/style-selector-v1.md)
...
- [writing-agent.md（写作 Agent 约定/协议）](docs/specs/writing-agent-v1.md)
...
- **底部（固定输入区：Composer）**
  ...
  - **意图选择（runIntent，开发期已落地）**：下拉 `自动 / 写作 / 改写 / 润色 / 分析 / 操作`
    - `自动`：后端启发式判断（兼容旧行为）
    - `写作/改写/润色`：更倾向启用写作闭环门禁（例如绑定风格库时的强闭环）
    - `分析/操作`：尽量避免误触写作强闭环（减少“挂着风格库但做别的事被误伤”）
    - **待定（UI 收敛）**：默认隐藏该下拉，仅在“误判/高级设置/需要硬覆盖”时出现；同时支持对话指令（例如 `/intent 写作|改写|润色|分析|操作|自动`）作为替代入口
  - **多模态输入**
    - **文本**：主输入框
...
- **知识库/素材库（MVP 先做最小闭环）**
  ...
  - **“卡片”不会乱套的关键：分层/分类型，而不是 200 篇=200 张同一种卡**
    ...
    - **Fingerprint（声音指纹，库体检）**：不要用“强硬口头禅规则”逼写法；用“率/分布/偏差”描述写作声音（通用、可解释、可回溯）
      - 指标口径必须同时支持：**每100句**（句式/节奏类）+ **每1000字**（密度类）
      - 必须包含：n‑gram 口癖 Top、问句率、短句率、口头语粒子率、第一/二人称密度、数字密度等
...
- **P0（写作 IDE 日用闭环）**
  ...
  - [x] Explorer（增强）：文件夹空目录展示、拖拽移动、搜索/过滤（树形目录 + drag&drop + 搜索框）
...
- **澄清策略（策略A，仍保留）：默认先问，但“澄清必须写进 todo”**
  - **硬约束**：Plan/Agent 模式下，第一步永远先 `run.setTodoList`。如果需要澄清，也要先把澄清问题写进 todo（最多 5 个高价值问题：平台画像/受众/目标/风格/素材来源等）。
  - 用户明确要求“先开始/先仿写看看/不要问”时：澄清条目标记为可跳过，并基于合理默认假设继续推进；同时把默认假设写入 Run 记录（Main Doc / Todo note）。
- **工程化执行纪律（把“prompt 里的话”落成可执行约束）**
  - **Tool = 能力边界**：模型只能使用系统当轮提供的工具；未提供则不得声称具备该能力（例如未提供联网检索工具时不得声称已上网）。
  - **KB = 工具**：引用 KB 必须经 `kb.search/kb.cite` 产出并可回链；不得凭空“KB 显示/KB 里有”。
  - **写入 proposal-first**：所有写入/批量变更默认“提案→Keep 执行→Undo 回撤”，避免模型嘴上说“已写入”但实际没落盘。
  - **Run 可回撤/可续跑**：每条用户消息保存 baseline（项目快照 + Main Doc + Todo），支持从历史回合继续（可选回滚或不回滚）。
  - **Context Pack 治理**：Main Doc 只存“主线摘要与约束”，不要把整篇原文塞进去；原文应通过 `@` 引用、项目文件读取或 KB 检索进入上下文（并带来源定位）。
  - **UI（Cursor-like）**：右侧顶部固定“Main Doc 一行摘要 + 置顶回合（默认折叠 4 行，可回撤/续跑）”；上滚跨过最近回合时自动切换置顶回合；过长内容默认折叠以保证输出可读。
...
- **选题/标题池**
  ...
  - **趋势/联网（可选开关）**
    ...
    - **重要（当前实现约束）**：在我们真正实现并向模型暴露联网工具（例如 `web.search/webSearch`）之前，**prompt/Agent 不得声称能上网**，也不得输出“来自网络”的引用；是否可联网以“本轮提供的工具列表”为准（工具=能力边界）。
- **平台画像驱动适配**
...
> 重要：示例里出现的 `webSearch/kb.ingest/...` 都只是“可能发生”的调用；实际是否调用、何时调用，完全由 LLM 根据任务与上下文决定。并且：当工具列表里**未提供联网工具**时，模型不得声称联网或引用网络来源。
